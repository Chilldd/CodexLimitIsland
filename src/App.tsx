...
import { selectIsland, sessionLabel, type Activity, type ActivitySnapshot, type LimitWindow, type Session, type UsageSnapshot } from "./islandState";
import "./App.css";
...
function mockSession(id: string, state: Activity, now: number): Session { return { id, parentSessionId: null, project: id, cwd: null, state, label: null, currentFile: "D:/src/VeryLongAuthServiceImplementation.cs", currentCommand: "dotnet test src/YuG.Api/YuG.Api.csproj", startedAt: now - 94000, lastActivityAt: now, completedAt: state === "completed" ? now : null, model: "Sol", reasoningLevel: "High", agents: [] }; }
...
function App() {
  ...
  const refreshUsage = useCallback(async () => { try { setUsage(await invoke<UsageSnapshot>("read_limits")); setUsageError(null); } catch (error) { setUsageError(String(error)); } }, []);
  useEffect(() => { void refreshUsage(); const timer = window.setInterval(() => void refreshUsage(), 60_000); return () => window.clearInterval(timer); }, [refreshUsage]);
  useEffect(() => { let mounted = true; let unlisten: (() => void) | undefined; void (async () => { try { unlisten = await listen<ActivitySnapshot>("activity-updated", event => { if (mounted) setActivity(event.payload); }); const next = await invoke<ActivitySnapshot>("read_activity"); if (mounted) setActivity(next); } catch (error) { console.error("会话状态接收失败", error); } })(); return () => { mounted = false; unlisten?.(); }; }, []);
  ...
  const header = view.mode === "minimal" ? "Codex idle" : view.mode === "multi-session" ? `${view.activeCount} sessions active` : view.mode === "attention" ? "Needs attention" : view.primary?.state === "completed" ? "Completed" : "Working";
  return <><main className={`island ${mode}`} style={expanded ? { height: expandedHeight } : undefined} onMouseEnter={onEnter} onMouseLeave={onLeave} onClick={() => expanded ? collapse() : void expand()} aria-label={view.mode === "minimal" ? `Codex 五小时剩余额度 ${percent(usage?.fiveHour ?? null)}` : view.label}>
    <div className="island-content"><div className="orb-wrap"><ThinkingOrb className="orb-small" state={view.orb.state} size={20} theme="dark" speed={view.orb.speed} paused={reducedMotion} aria-hidden="true" /><ThinkingOrb className="orb-large" state={view.orb.state} size={64} theme="dark" speed={view.orb.speed} paused={reducedMotion} aria-hidden="true" /></div>
