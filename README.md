...
Codex Hook 从标准输入将 JSON 交给同一个 `CodexLimitIsland.exe --hook`。该瞬时进程将原始事件 POST 到灵动岛监听的 `127.0.0.1:17321/api/codex/hook`；灵动岛尚未运行时会启动 GUI 并在有限时间内重试当前事件。Rust 后端仅在内存中保存状态所需字段，并通过 Tauri 事件实时通知 React。Hook 命令配置在用户级 `%USERPROFILE%\.codex\hooks.json`，修改后需在 Codex 中重新信任并重启。
